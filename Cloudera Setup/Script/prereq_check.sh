...
function check_java_jre() {
    local _java="$(type -p java)"
    if [ -f $_java ]; then
        version=$("$_java" -version 2>&1 | awk -F '"' '/version/ {print $2}')
        versionMajor=$(echo "$version" | awk -F '.' '{print $1}')

        if [ $versionMajor -eq 8 ] || [ $versionMajor -eq 11 ] || [ $versionMajor -eq 17 ]; then
            state "Java: Found JRE Version Major $versionMajor" 0
        else
            state "Java: JRE version is not supported $version" 1
        fi
    else
        state "Java: No JRE found" 1
    fi
}

function check_java_sdk() {
    local _javac="$(type -p javac)"
    if [ -f $_javac ]; then
        version=$("$_javac" -version 2>&1 | awk -F ' ' '/javac/ {print $2}')
        versionMajor=$(echo "$version" | awk -F '.' '{print $1}')

        if [ $versionMajor -eq 8 ] || [ $versionMajor -eq 11 ] || [ $versionMajor -eq 17 ]; then
            state "Java: Found SDK Version Major $versionMajor" 0
        else
            state "Java: SDK version is not supported $version" 1
        fi
    else
        state "Java: No JDK Found" 1
    fi
}
...
function check_jdbc_connector() {
    # See Install and Configure Databases
    # https://docs.cloudera.com/cdp-private-cloud-base/7.1.9/installation/topics/cdpdc-install-configure-databases.html
    local mysql_connector=/usr/share/java/mysql-connector-java.jar
    local postgres_connector=/usr/share/java/postgresql-connector-java.jar
    local oracle_connector=/usr/share/java/oracle-connector-java.jar
    if [ -f $mysql_connector ]; then
        state "Database: MySQL JDBC Driver is installed" 0
    elif [ -f $postgres_connector ]; then
        state "Database: PostgreSQL JDBC Driver is installed" 0
    elif [ -f $oracle_connector ]; then
        state "Database: Oracle JDBC Driver is installed" 0
    else
        state "Database: No JDBC Driver is not installed" 1
    fi
}
